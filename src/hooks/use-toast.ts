...
// Map your custom type to the library type
...
const addToRemoveQueue = (toastId: string, dispatch: (action: Action) => void) => {
  ...
  const timeout = setTimeout(() => {
    ...
    dispatch({ type: "REMOVE_TOAST", toastId }); // <-- fixed: closed object and parentheses
  }, TOAST_REMOVE_DELAY);
  ...
  switch (action.type) {
    ...
    case "DISMISS_TOAST": {
      ...
      state.toasts.forEach((t) => {
        if (!toastId || t.id === toastId) addToRemoveQueue(t.id, dispatch);
      });
