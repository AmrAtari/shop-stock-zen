...
const OpeningStockEntry = () => {
    ...
    const isReadyToGenerate = selectedStore && inventoryAssetAccount && equityOpeningBalanceAccount && totalStockValue > 0;
            ...
            <Card>
                ...
                <CardContent className="space-y-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <Select value={equityOpeningBalanceAccount} onValueChange={setEquityOpeningBalanceAccount} disabled={isLoading}>
                    ...
                    <Table>
                        ...
                        <TableBody>
                            <TableRow>
                                ...
                                <TableCell className="text-right font-mono text-lg">{formatCurrency(totalStockValue, currency)}</TableCell>
                                <TableCell className="text-right font-mono text-lg">—</TableCell>
                            </TableRow>
                            <TableRow>
                                <TableCell>2</TableCell>
                                <TableCell className="font-semibold">
                                    {accounts?.find(a => a.id === equityOpeningBalanceAccount)?.account_name || '— Opening Balance Equity'}
                                </TableCell>
                                <TableCell className="text-right font-mono text-lg">—</TableCell>
                                <TableCell className="text-right font-mono text-lg">{formatCurrency(totalStockValue, currency)}</TableCell>
                            </TableRow>
                ...
                <CardContent className="pt-6">
                    {!isReadyToGenerate && (
                        <div className="text-red-600 font-bold border-l-4 border-red-600 pl-3 flex items-center gap-2">
                            ...
                            <p>**REQUIRED:** Please select a Store, Inventory Asset Account, and Opening Equity Account to proceed.</p>
                        </div>
                    )}
                    {totalStockValue === 0 && isReadyToGenerate && (
                        <div className="text-orange-600 font-bold border-l-4 border-orange-600 pl-3 flex items-center gap-2">
                            <AlertTriangle className="w-5 h-5 flex-shrink-0" />
                            <p>**WARNING:** The calculated Total Stock Value is zero. The resulting journal entry will have zero amounts.</p>
                        </div>
                    )}
                </CardContent>
            </Card>
