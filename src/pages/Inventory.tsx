...
import {
  Dialog,
  DialogContent,import { useState, useMemo } from "react";
import { Plus, Search, Edit, Trash2, Upload, Download, History, Clipboard } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
...
const InventoryNew = () => {
  ...
  const filteredInventory = useMemo(() => {
    ...
    return inventory.filter((item: Item) => {
      ...
      return (
        matchesSearch && matchesModelNumber && matchesSeason && matchesMainGroup && matchesCategory
      );
    });
  ...
  const uniqueLocations = useMemo(
    () => stores.map((store: { id: string, name: string }) => ({ id: store.id, name: store.name })).sort((a, b) => a.name.localeCompare(b.name)),
    [stores],
  );

  const uniqueSeasons = useMemo(() => [...new Set((inventory as Item[]).map((i) => i.season).filter(Boolean))].sort(), [inventory]);
