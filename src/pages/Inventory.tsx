...
export const Inventory: React.FC = () => {
  ...
  /**
   * ✅ Fetch store inventory + calculate real quantity
   * by combining stock + received purchase order items.
   */
  const fetchInventory = async () => {
    try {
      setLoading(true);

      // Fetch main inventory data
      const { data: inventoryData, error: invError } = await supabase.from("store_inventory").select(
        `
          id,
          quantity,
          created_at,
      ...
      // Fetch received purchase order items
      ...
      (poItems || []).forEach((po) => {
        ...
        receivedMap[key] = (receivedMap[key] || 0) + (po.received_quantity || 0);
      });

      // Normalize and calculate final quantity
      const normalizedData: InventoryItem[] = (inventoryData || []).map((row: any) => {
        const item = Array.isArray(row.items) ? row.items[0] : row.items;
        const store = Array.isArray(row.stores) ? row.stores[0] : row.stores;

        const baseQty = Number(row.quantity) || 0;
        const receivedQty = item ? receivedMap[item.id] || 0 : 0;
        const totalQty = baseQty + receivedQty;
